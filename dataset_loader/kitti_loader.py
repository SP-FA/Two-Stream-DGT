...
class KITTI_Loader(BaseLoader):
    ...
    def __getitem__(self, idx):
        ...
        _, _, claLabel = searchArea.points_in_box(trueBox, returnMask=True, center=True)

        return {
            "template": template.convert2Tensor(),
            "search": searchArea.convert2Tensor(),
            "claLabel": torch.tensor(claLabel).float(),
            "trueBox": torch.tensor([trueBox.center[0], trueBox.center[1], trueBox.center[2],
                                     trueBox.wlh[0], trueBox.wlh[1], trueBox.wlh[2], -searchOffset[2]], dtype=torch.float).view(-1)
        }
